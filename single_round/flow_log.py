...
logger = logging.getLogger("single_round.flow")

_request_id: ContextVar[str | None] = ContextVar("request_id", default=None)
_depth = threading.local()


def set_flow_context(request_id: UUID | str | None) -> None:
    _request_id.set(str(request_id) if request_id else None)
...
def log_request_start(request_id: UUID, content: str) -> None:
    set_flow_context(request_id)
    logger.info("%sREQUEST user: %s", _request_prefix(), _truncate(content, 120))


def log_request_end() -> None:
    logger.info("%sREQUEST done", _request_prefix())
